...
## Cross-Language Format Mapping (GAP-46, GAP-47)

### Python canonical format
```
OUTCOME_DURATION_PROXIMITY_CALENDAR_DIRECTION_GENERATION[_SUFFIX]
```
- Separator: underscore (`_`)
- Field order: outcome, duration, proximity, calendar, direction, generation
- Suffix algorithm: SHA-256 truncated to 8 chars (authoritative)

### MQL4 reentry_key format
```
SIG~TB~OB~PB~G
```
- Separator: tilde (`~`)
- Field mapping:
  - `SIG` = calendar_id (NOT outcome — **field order differs**)
  - `TB` = duration
  - `OB` = outcome
  - `PB` = proximity
  - `G` = generation
- NOTE: `direction` is NOT present in MQL4 reentry_key

### Comment suffix algorithm conflict

| Implementation | Algorithm | Status |
|---|---|---|
| Python | SHA-256 (authoritative) | Use always |
| MQL4 | FNV32→Base32 (local fallback) | Use only if CSV suffix field empty |

**Resolution**: Python-generated suffix must be passed through CSV to EA. MQL4 local suffix is used only when the CSV `suffix` field is empty.

### Cross-language field order table

| Python field | Python position | MQL4 field | MQL4 position |
|---|---|---|---|
| outcome | 0 | OB | 2 |
| duration | 1 | TB | 1 |
| proximity | 2 | PB | 3 |
