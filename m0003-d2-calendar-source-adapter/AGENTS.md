# Agent instructions -- Calendar Source Adapter (D2_CALENDAR_SOURCE_ADAPTER)
...
- Primary module root: `m0003-d2-calendar-source-adapter`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0003-d2-calendar-source-adapter-src/`, `m0003-d2-calendar-source-adapter-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-D2_CALENDAR_SOURCE_ADAPTER
- Applicable rules: VAL-S03-001
