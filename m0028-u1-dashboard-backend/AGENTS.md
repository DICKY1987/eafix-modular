# Agent instructions -- Dashboard Backend (U1_DASHBOARD_BACKEND)
...
- Primary module root: `services/dashboard-backend`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0028-u1-dashboard-backend-src/`, `m0028-u1-dashboard-backend-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-U1_DASHBOARD_BACKEND
- Applicable rules: VAL-U1_DASHBOARD_BACKEND-NEEDS_REVIEW
