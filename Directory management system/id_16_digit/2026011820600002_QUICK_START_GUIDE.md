...
# Include optional validators
python automation/2026012120420011_registry_cli.py validate --include-module --include-process

# Generate detailed report
python automation/2026012120420011_registry_cli.py validate --report validation_report.json
```
...
# Export to SQLite (queryable)
python automation/2026012120420011_registry_cli.py export --format sqlite --output registry.sqlite

# Query SQLite
sqlite3 registry.sqlite "SELECT filename, extension FROM entity_records WHERE entity_kind='file' AND extension='py'"
```
...
```bash
# 1. Export to SQLite
python automation/2026012120420011_registry_cli.py export --format sqlite --output analysis.sqlite

# 2. Query (example: count files by module)
sqlite3 analysis.sqlite "SELECT module_id, COUNT(*) as file_count FROM entity_records WHERE entity_kind='file' GROUP BY module_id"

# 3. Export specific subset to CSV
python automation/2026012120420011_registry_cli.py export --format csv --output core_files.csv --entity-kind file
```

See: `HARDENING_QUICK_REFERENCE.md` for complete command reference and troubleshooting.
