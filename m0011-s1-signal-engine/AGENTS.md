# Agent instructions -- Signal Engine (S1_SIGNAL_ENGINE)
...
- Primary module root: `services/signal-generator`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0011-s1-signal-engine-src/`, `m0011-s1-signal-engine-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-S1_SIGNAL_ENGINE
- Applicable rules: VAL-S11-001
