# Agent instructions -- Trade Close Classifier (O3_TRADE_CLOSE_CLASSIFIER)
...
- Primary module root: `m0020-o3-trade-close-classifier`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0020-o3-trade-close-classifier-src/`, `m0020-o3-trade-close-classifier-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-O3_TRADE_CLOSE_CLASSIFIER
- Applicable rules: VAL-S20-001
