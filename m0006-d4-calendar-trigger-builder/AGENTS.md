# Agent instructions -- Calendar Trigger Builder (D4_CALENDAR_TRIGGER_BUILDER)
...
- Primary module root: `m0006-d4-calendar-trigger-builder`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0006-d4-calendar-trigger-builder-src/`, `m0006-d4-calendar-trigger-builder-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-D4_CALENDAR_TRIGGER_BUILDER
- Applicable rules: VAL-S06-001
