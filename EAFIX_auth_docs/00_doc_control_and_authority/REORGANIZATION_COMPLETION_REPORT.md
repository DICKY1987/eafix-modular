...
| `doc_authority.json` entry for `DOC-PAT-EVERY-REUSABLE-PATTERN-PATTERN-304__every_reusable_pattern.pattern.yaml` | File referenced in summary router with `status: missing_or_external_pattern_stub`. Not a doc_authority concern; listed in summary router only. |
