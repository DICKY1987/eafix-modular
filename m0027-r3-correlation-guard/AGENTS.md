# Agent instructions -- Correlation Guard (R3_CORRELATION_GUARD)
...
- Primary module root: `shared/reentry`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0027-r3-correlation-guard-src/`, `m0027-r3-correlation-guard-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-R3_CORRELATION_GUARD
- Applicable rules: VAL-R3_CORRELATION_GUARD-NEEDS_REVIEW
