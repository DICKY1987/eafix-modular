# Agent instructions -- Flow Orchestrator (F4_FLOW_ORCHESTRATOR)
...
- Primary module root: `m0025-f4-flow-orchestrator`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0025-f4-flow-orchestrator-src/`, `m0025-f4-flow-orchestrator-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-F4_FLOW_ORCHESTRATOR
- Applicable rules: VAL-S25-001
