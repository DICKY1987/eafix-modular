# Agent instructions -- Configuration Service (F1_CONFIG_PREFERENCES)
...
- Primary module root: `m0001-f1-config-preferences`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0001-f1-config-preferences-src/`, `m0001-f1-config-preferences-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-F1_CONFIG_PREFERENCES
- Applicable rules: VAL-S01-001
