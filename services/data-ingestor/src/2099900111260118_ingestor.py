...
class DataIngestor:
    ...
    def __init__(self, settings: Settings, metrics=None):
        ...
        if self.settings.enable_dde:
            try:
                ...
                logger.warning("MT4 adapter not available", error=str(exc))
            else:
        ...
        if self.settings.enable_csv:
            try:
                ...
                logger.warning("CSV adapter not available", error=str(exc))
            else:
        ...
        if self.settings.enable_socket:
            try:
                ...
                logger.warning("Socket adapter not available", error=str(exc))
            else:
    ...
    async def start(self):
        ...
        try:
            ...
            logger.warning("Redis unavailable, continuing without pub/sub", error=str(e))
