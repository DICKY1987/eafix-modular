...
class ReentryEnginePlugin(BasePlugin):
    ...
    async def _on_initialize(self) -> None:
        ...
        self._processor = TradeResultProcessor(settings, self._metrics)
    ...
    def _handle_trade_result(self, event_type: str, data: Any, source: str) -> None:
        ...
        if not self._processor:
            logger.error("Trade result processor not initialized")
            return
        asyncio.create_task(self._process_trade_result(data))

    async def _process_trade_result(self, data: Any) -> None:
        """Process trade result and emit decision event."""
        try:
            decision = await self._processor.process_trade_result(data)
            if decision:
                self.emit_event(
                    "reentry_decision",
                    {
            ...
            logger.error(f"Error processing trade result: {exc}")
