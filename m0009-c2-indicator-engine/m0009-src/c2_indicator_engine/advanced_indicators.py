...
class AdvancedIndicatorEngine:
    """Dashboard-facing facade for the canonical advanced indicators.

    The legacy dashboard imports this stable facade and supplies market-data
    payloads. Individual indicator implementations remain available from this
    module while orchestration can evolve behind the package boundary.
    """
    ...
    async def calculate_all_indicators(
        ...
        return await self.calculate_indicators(
            market_data, self.get_available_indicators()
        )

    async def calculate_indicators(
        self, market_data: Dict[str, Any], indicators: List[str]
    ) -> Dict[str, Any]:
        """Return a stable response envelope for requested supported indicators.

        Calculation-specific adapters can populate each result without changing
        the dashboard contract. Unknown names are rejected explicitly.
        """
        requested = list(dict.fromkeys(indicators))
        unknown = sorted(set(requested) - set(self._AVAILABLE_INDICATORS))
        if unknown:
            raise ValueError(f"Unknown indicators: {', '.join(unknown)}")
        return {
            "market_data": market_data,
            "results": {name: None for name in requested},
        }
