# Agent instructions -- Indicator Engine (C2_INDICATOR_ENGINE)
...
- Primary module root: `services/indicator-engine`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0009-c2-indicator-engine-src/`, `m0009-c2-indicator-engine-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-C2_INDICATOR_ENGINE
- Applicable rules: VAL-S09-001
