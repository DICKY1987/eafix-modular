# Agent instructions -- MT4 Expiry Zones Overlay (U3_MT4_EXPIRY_OVERLAY)
...
- Primary module root: `services/desktop-ui`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0030-u3-mt4-expiry-overlay-src/`, `m0030-u3-mt4-expiry-overlay-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-U3_MT4_EXPIRY_OVERLAY
- Applicable rules: VAL-U3_MT4_EXPIRY_OVERLAY-NEEDS_REVIEW
