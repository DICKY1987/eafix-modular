# Agent instructions -- Desktop Operator UI (U4_DESKTOP_OPERATOR)
...
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0031-u4-desktop-operator-src/`, `m0031-u4-desktop-operator-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-U4_DESKTOP_OPERATOR
- Applicable rules: VAL-U4_DESKTOP_OPERATOR-NEEDS_REVIEW
