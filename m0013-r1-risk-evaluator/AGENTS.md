# Agent instructions -- Risk Evaluator (R1_RISK_EVALUATOR)
...
- Primary module root: `services/risk-manager`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0013-r1-risk-evaluator-src/`, `m0013-r1-risk-evaluator-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-R1_RISK_EVALUATOR
- Applicable rules: VAL-S13-001
