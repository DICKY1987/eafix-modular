# Agent instructions -- Idempotency (SK2_IDEMPOTENCY)
...
- Primary module root: `shared/idempotency`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0034-sk2-idempotency-src/`, `m0034-sk2-idempotency-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-SK2_IDEMPOTENCY
- Applicable rules: VAL-SK2_IDEMPOTENCY-NEEDS_REVIEW
