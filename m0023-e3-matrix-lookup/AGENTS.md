# Agent instructions -- Matrix Lookup (E3_MATRIX_LOOKUP)
...
- Primary module root: `services/reentry-matrix-svc`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0023-e3-matrix-lookup-src/`, `m0023-e3-matrix-lookup-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-E3_MATRIX_LOOKUP
- Applicable rules: VAL-S23-001
