...
BASE_REF_REQUIRED_GATES = {"STEP-04","STEP-05","STEP-06","STEP-07","STEP-08","STEP-09"}
...
def git_path_exists(root, ref, path):
...
def conflict_resolution_state(c): return c.get("resolution_state") or "unresolved"

def accepted_conflict(c):
    return conflict_resolution_state(c).startswith("accepted_")

def accepted_with_cutover_clearance(c):
    state = conflict_resolution_state(c)
    if state in {"accepted_migrated","accepted_nonblocking_pending_migration"}:
        return True
    return state == "accepted_cutover_waiver" and bool(c.get("decision_reference"))

def iter_record_path_refs(record):
    for ref in record.get("source_refs") or []:
        yield ("source_refs", ref.get("path"), ref.get("commit_sha"))
    for ref in record.get("evidence_refs") or []:
        if isinstance(ref, str):
            yield ("evidence_refs", ref, None)
        elif isinstance(ref, dict):
            yield ("evidence_refs", ref.get("path"), ref.get("commit_sha"))
        else:
            yield ("evidence_refs", None, None)
    for field, meta in (record.get("field_resolution") or {}).items():
        for ref in meta.get("evidence_refs") or []:
            if isinstance(ref, str):
                yield (f"field_resolution.{field}.evidence_refs", ref, None)
            elif isinstance(ref, dict):
                yield (f"field_resolution.{field}.evidence_refs", ref.get("path"), ref.get("commit_sha"))
            else:
                yield (f"field_resolution.{field}.evidence_refs", None, None)

def check_provenance_paths(root, records, res, gate):
    if not stage_at_least(gate,"STEP-06"): return
    unresolved=[]; malformed=[]
    for record in records:
        for loc, path, commit_sha in iter_record_path_refs(record):
            if not isinstance(path, str) or not path:
                malformed.append((record.get("step_code"), loc, path))
                continue
            if (root/path).exists(): continue
            if commit_sha and git_path_exists(root, commit_sha, path): continue
            unresolved.append((record.get("step_code"), loc, path, commit_sha))
    res.add("PROV-01-path-refs-well-formed",not malformed,f"malformed={malformed[:50]}")
    res.add("PROV-02-path-refs-resolvable",not unresolved,f"unresolved={unresolved[:50]}")
...
def check_conflicts(root,records,res,gate):
    ...
    shape_errors=[]; unresolved=[]; blocking=[]; contradictory=[]
    required_fields=("conflict_id","record_id","review_decision","process_material","blocking_scope","migration_action","decision_reference","validation_evidence","resolution_state","status","authority_status")
    for c in cs:
        for field in required_fields:
            if field not in c:
                shape_errors.append((c.get("record_id"), field, "missing"))
        state = conflict_resolution_state(c)
        if accepted_conflict(c) and (c.get("status") == "conflicted" or c.get("authority_status") == "needs_review"):
            contradictory.append((c.get("conflict_id"), c.get("status"), c.get("authority_status"), state))
    for c in relevant:
        if not c.get("review_decision") or conflict_resolution_state(c) == "unresolved":
            unresolved.append(c.get("conflict_id"))
        if stage_at_least(gate,"STEP-08") and c.get("blocking_scope") == "process_cutover" and not accepted_with_cutover_clearance(c):
            blocking.append((c.get("conflict_id"), conflict_resolution_state(c), c.get("decision_reference")))
    res.add("CONF-00-record-shape",not shape_errors,f"errors={shape_errors[:50]}")
    res.add("CONF-01-readable",True,f"total={len(cs)} material={len(relevant)} unresolved_material={len(unresolved)}",severity="INFO")
    if stage_at_least(gate,"STEP-07"): res.add("CONF-02-material-adjudicated",not unresolved,f"unresolved={unresolved}")
    res.add("CONF-03-state-consistency",not contradictory,f"contradictory={contradictory[:50]}")
    if stage_at_least(gate,"STEP-08"): res.add("CONF-04-cutover-blockers-cleared",not blocking,f"blocking={blocking[:50]}")
...
def main():
    ...
    if args.gate in BASE_REF_REQUIRED_GATES and not args.base_ref:
        abort("--base-ref is required for STEP-04 through STEP-09")
    reg=root/REG_JSONL
    ...
    check_provenance_paths(root,records,res,args.gate)
