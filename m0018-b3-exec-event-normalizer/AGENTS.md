# Agent instructions -- Exec Event Normalizer (B3_EXEC_EVENT_NORMALIZER)
...
- Primary module root: `services/event-gateway`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0018-b3-exec-event-normalizer-src/`, `m0018-b3-exec-event-normalizer-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-B3_EXEC_EVENT_NORMALIZER
- Applicable rules: VAL-S18-001
