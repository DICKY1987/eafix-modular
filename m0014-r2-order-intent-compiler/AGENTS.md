# Agent instructions -- Order Intent Compiler (R2_ORDER_INTENT_COMPILER)
...
- Primary module root: `m0014-r2-order-intent-compiler`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0014-r2-order-intent-compiler-src/`, `m0014-r2-order-intent-compiler-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-R2_ORDER_INTENT_COMPILER
- Applicable rules: VAL-S14-001
