# Agent instructions -- Market Feed Adapter (D1_MARKET_FEED_ADAPTER)
...
- Primary module root: `services/data-ingestor`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0007-d1-market-feed-adapter-src/`, `m0007-d1-market-feed-adapter-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-D1_MARKET_FEED_ADAPTER
- Applicable rules: VAL-S07-001
