# Agent instructions -- Clock Scheduler (F3_CLOCK_SCHEDULER)
...
- Primary module root: `m0002-f3-clock-scheduler`
- If `module_root` is this module's own scaffold folder (not yet a real service directory), this module's canonical
  ...
  (`m0002-f3-clock-scheduler-src/`, `m0002-f3-clock-scheduler-tests/`, etc.) already contain the real implementation -- check `file_ownership.owned_files`
  in `manifest.json` first.

## Validation

- Applicable gates: GATE-F3_CLOCK_SCHEDULER
- Applicable rules: VAL-S02-001
